...
This repository is the agent source. The OpenWrt package that installs it —
Makefile, dependencies, `conffiles`, the sysupgrade keep list — lives in
[ddimension/openwrt-repo](https://github.com/ddimension/openwrt-repo) under
`apman/` and pulls a pinned commit from here. Changing the agent therefore
means a commit here and a `PKG_SOURCE_VERSION` bump there; `contrib/release.sh`
does both and can roll the result straight onto a development access point.

The layout mirrors the target filesystem: everything under `files/` is
installed at the same path on the access point.
...
This repository starts on 2026-08-22, when the agent was split out. Everything
before that is in [ddimension/openwrt-repo](https://github.com/ddimension/openwrt-repo)
under `apman/` — including `apman-status-curl`, an HTTP POST variant of the
bridge (options `url_event` / `url_status`) that was removed because the
